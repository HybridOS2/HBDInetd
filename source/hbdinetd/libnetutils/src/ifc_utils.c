...
in_addr_t ifc_ipv4_prefix_length_to_netmask(int prefix_length)
{
    in_addr_t mask = 0;
...
int ifc_ipv4_netmask_to_prefix_length(in_addr_t mask)
{
    int prefix_length = 0;
...
const char *ifc_ipaddr_to_string(in_addr_t addr)
{
...
int ifc_act_on_address(int action, const char *name, const char *address,
    ...
    ret = ifc_string_to_ip(address, &ss);
    if (ret) {
        return ret;
    }
...
int ifc_clear_ipv6_addresses(const char *name) {
    ...
    while (fscanf(f, "%32s %*02x %02x %*02x %*02x %63s\n",
        ...
        if (ret) {
            HLOG_ERR("Deleting address %s/%d on %s: %s", addrstr, prefixlen, ifname,
                 strerror(-ret));
            lasterror = ret;
        }
    }
...
int ifc_set_prefix_length(const char *name, int prefix_length)
{
    struct ifreq ifr;
    // TODO - support ipv6
    if (prefix_length > 32 || prefix_length < 0) return -1;

    in_addr_t mask = ifc_ipv4_prefix_length_to_netmask(prefix_length);
    ifc_init_ifr(name, &ifr);
    init_sockaddr_in(&ifr.ifr_addr, mask);
...
int ifc_get_info(const char *name, in_addr_t *addr, int *prefix_length, unsigned *flags)
{
    struct ifreq ifr;
    ifc_init_ifr(name, &ifr);
    ...
    if (prefix_length != NULL) {
        if(ioctl(ifc_ctl_sock, SIOCGIFNETMASK, &ifr) < 0) {
            ...
            *prefix_length = ifc_ipv4_netmask_to_prefix_length((int)
      ...
      struct in_addr gw)
{
    ...
    netmask = ifc_ipv4_prefix_length_to_netmask(prefix_length);
      ...
      struct in6_addr gw)
{
    ...
    if (ifindex == 0) {
        HLOG_ERR("if_nametoindex() failed: interface %s\n", ifname);
        return -ENXIO;
    }
        ...
        const char *gw)
{
    ...
    if (ret != 0) {
        HLOG_ERR("getaddrinfo failed: invalid address %s\n", dst);
        return -EINVAL;
    }
    ...
    if (((addr_ai->ai_family == AF_INET6) && (prefix_length < 0 || prefix_length > 128)) ||
        ...
        HLOG_ERR("ifc_add_route: invalid prefix length");
    ...
    if (ret != 0) {
        HLOG_ERR("getaddrinfo failed: invalid gateway %s\n", gw);
        freeaddrinfo(addr_ai);
        return -EINVAL;
    }

    if (addr_ai->ai_family != gw_ai->ai_family) {
        HLOG_ERR("ifc_add_route: different address families: %s and %s\n", dst, gw);
        freeaddrinfo(addr_ai);
        freeaddrinfo(gw_ai);
        return -EINVAL;
    }
    ...
    if (addr_ai->ai_family == AF_INET6) {
        ...
        HLOG_ERR("ifc_add_route: getaddrinfo returned un supported address family %d\n",
                  addr_ai->ai_family);
        ret = -EAFNOSUPPORT;
    }
...
ifc_configure(const char *ifname,
    ...
    if (ifc_up(ifname)) {
        HLOG_ERR("failed to turn on interface %s: %s\n", ifname, strerror(errno));
        ifc_close();
        return -1;
    }
    if (ifc_set_addr(ifname, address)) {
        HLOG_ERR("failed to set ipaddr %s: %s\n", ifc_ipaddr_to_string(address), strerror(errno));
        ifc_close();
        return -1;
    }
    if (ifc_set_prefix_length(ifname, prefix_length)) {
        HLOG_ERR("failed to set prefix_length %d: %s\n", prefix_length, strerror(errno));
        ifc_close();
        return -1;
    }
    if (ifc_create_default_route(ifname, gateway)) {
        HLOG_ERR("failed to set default route %s: %s\n", ifc_ipaddr_to_string(gateway), strerror(errno));
        ifc_close();
        return -1;
    }
    ...
    property_set(dns_prop_name, dns1 ? ifc_ipaddr_to_string(dns1) : "");
    snprintf(dns_prop_name, sizeof(dns_prop_name), "net.%s.dns2", ifname);
    property_set(dns_prop_name, dns2 ? ifc_ipaddr_to_string(dns2) : "");
    snprintf(dns_prop_name, sizeof(dns_prop_name), "net.%s.gw", ifname);
    property_set(dns_prop_name, gateway ? ifc_ipaddr_to_string(gateway) : "");
#else
    (void)dns1;
    (void)dns2;
#endif
